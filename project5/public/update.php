...
UPDATE h_hospitalization SET Conditions = '{$_POST["Cond"]}' WHERE ID='{$_POST["ID"]}-{$_POST["ID2"]}'");
if($delete == false) {
    echo "<script>alert(\"환자 명단에 존재하지 않습니다.\"); location.href='../home';</script>";
} else {
echo "<script>alert(\"상태가 변경되었습니다.\"); location.href='../home';</script>";
}
