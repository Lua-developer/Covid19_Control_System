...
$userID = $_POST["userID"];
$userPS = $_POST["userPS"];
$sql = "SELECT * FROM admin WHERE id='$userID' && pass ='$userPS'";
// 아이디, 패스워드 받아오는 SQL 구문입니다.
$login_set = mysqli_query($connection, $sql);
...
if(mysqli_fetch_array($login_set)) {
    ...
    echo "<script>alert(\"관리자님! 환영합니다.\"); location.href='../home';</script>";
} else {
    echo "<script>alert(\"로그인 실패! 다시 시도하십시요.\"); location.href='../login';</script>";
}
